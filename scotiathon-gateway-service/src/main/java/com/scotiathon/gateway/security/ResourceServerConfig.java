...
public class ResourceServerConfig extends ResourceServerConfigurerAdapter {
	...
	@Value("${auth.supervisor.list}")
	private String[] urlSupervisor;

	@Override
	public void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers(authWhiteList).permitAll().antMatchers(urlAlwaysDeny)
				.denyAll().antMatchers(HttpMethod.POST, urlSupervisor).hasAuthority("SUPERVISOR").anyRequest()
				.authenticated().and().cors().configurationSource(corsConfigurationSource()).and().exceptionHandling()
