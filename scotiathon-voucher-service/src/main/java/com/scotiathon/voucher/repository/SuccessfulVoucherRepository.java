...
public interface SuccessfulVoucherRepository extends JpaRepository<PassedVoucher, Long> {
	...
	@Query(value = "SELECT * FROM passed_vouchers pv WHERE pv.assigned_to_clerk=:clientId AND (pv.status_clerk NOT IN (:statusClerkNotIn) OR pv.status_clerk IS NULL) ORDER BY pv.invoice_date", nativeQuery = true)
	public List<PassedVoucher> getClerkVouchers(Long clientId, List<String> statusClerkNotIn);

	@Modifying(clearAutomatically = true)
	@Query(value = "UPDATE passed_vouchers pv SET pv.started_at =NOW() WHERE pv.id=:voucherId", nativeQuery = true)
	public void setStartTime(Long voucherId);
